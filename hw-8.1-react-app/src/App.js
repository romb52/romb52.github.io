...
function App() {
  ...
  const [openSubMenu, setOpenSubMenu] = useState({ isOpen: false, link: '' });
  ...
  useEffect(() => {
    const currentMenuItem = navMenu.find(item => item.link === location.pathname);
    if (currentMenuItem && currentMenuItem.submenu === null) {
      setOpenSubMenu({ isOpen: false, link: '' });
    }
  }, [location.pathname]);
    ...
    <div className="App">
      <header className="App-header">
        <div className="container">
          ...
          <button className='menuBtn' onClick={toggleMobileMenu}><IoMenu style={{ fontSize: '30px', backgroundColor: 'BFBFBF', border: 'none' }} /></button>
          <nav className={`mobileMenu ${mobileMenuOpen ? 'active' : ''}`}>
            <ul >
              {Array.isArray(navMenu) && navMenu.map(item => (
                <li className='parent' key={item.link}>
                  <div className='menyItem'>
